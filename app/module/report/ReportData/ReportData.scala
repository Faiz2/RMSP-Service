...
trait ReportData {
	...
	def conditionAllot(data: JsValue): DBObject  = {
		val builder = MongoDBObject.newBuilder
		(data \ "condition" \ "cycle").asOpt[String].map(x => builder += "phase" -> List(x)).getOrElse(Unit)
		(data \ "condition" \ "hospital").asOpt[String].map(x => builder += "report4_mod1.医院" -> x).getOrElse(Unit)
		(data \ "condition" \ "dimension").asOpt[String].map(x => builder += "report4_mod1.因素" -> x).getOrElse(Unit)
		builder.result
	}
	...
	def conditionSalesCustomer(data: JsValue): DBObject = {
		val builder = MongoDBObject.newBuilder
		(data \ "condition" \ "cycle").asOpt[String].map(x => builder += "phase" -> List(x)).getOrElse(Unit)
		(data \ "condition" \ "hospital").asOpt[String].map(x => builder += "report5_mod1.医院" -> x).getOrElse(Unit)
		(data \ "condition" \ "product").asOpt[String].map(x => builder += "report5_mod1.产品" -> x).getOrElse(Unit)
		builder.result
	}
	...
	def conditionSalesDeputy(data: JsValue): DBObject = {
		val builder = MongoDBObject.newBuilder
		(data \ "condition" \ "cycle").asOpt[String].map(x => builder += "phase" -> List(x)).getOrElse(Unit)
		(data \ "condition" \ "people").asOpt[String].map(x => builder += "report5_mod2.销售代表" -> x).getOrElse(Unit)
		(data \ "condition" \ "product").asOpt[String].map(x => builder += "report5_mod2.产品" -> x).getOrElse(Unit)
		builder.result
	}
...
//		val phase = obj.getAs[List[String]]("phase").getOrElse("")
		val reDocObj = obj.getAs[List[BasicDBObject]]("report1_mod1").getOrElse(Nil).
	...
	val d2mMarketSalesPerformance: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report1_mod2").getOrElse(Nil).
						map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}
	...
	val d2mDeputyTimerAllot: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report2_mod1").getOrElse(Nil).
						map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mDeputyProductInformation: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report2_mod2").getOrElse(Nil).
						map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mDeputyEmpiric: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report2_mod3").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mDeputySalesSkills: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report2_mod4").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mDeputyWorkAttitude: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report2_mod5").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}
	...
	val d2mManagerCost: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report3_mod1").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2ManagerTimerAllot: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report3_mod2").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}
	...
	val d2mAllot: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report4_mod1").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}
	...
	val d2mSalesCustomer: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report5_mod1").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mSalesDeputy: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report5_mod2").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}

	val d2mSalesProduct: DBObject => resultType = { obj =>
		val reDocObj = obj.getAs[List[BasicDBObject]]("report5_mod3").getOrElse(Nil).
			map(x => x.toMap.toMap.map(f => f._1.toString -> f._2.toString))
		Map("result" -> toJson(reDocObj))
	}
}
